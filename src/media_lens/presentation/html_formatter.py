...
def generate_html(job_dirs: List[Path], sites: list[str], template_dir_path: Path, template_name: str) -> str:
    logger.info(f'Generating HTML for {len(job_dirs)} jobs and {len(sites)} sites')
    content: Dict = {
    ...
    for node in job_dirs_root.iterdir():
        if node.is_dir():
            if re.match(UTC_PATTERN, node.name):
                dirs.append(node)
    return generate_html(dirs, sites, template_dir_path, template_name)
